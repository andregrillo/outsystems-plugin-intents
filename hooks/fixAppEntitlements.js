...
function replacePlaceholdersInPlist(plistPath, placeHolderValues) {
    ...
    //Entitlements - Enabling Keychain Sharing support 
    //plistContents = plistContents.replace(/<dict>/g, '<dict>\n\t<key>keychain-access-groups</key>\n\t<array>\n\t\t<string>$(AppIdentifierPrefix)keychain.__APP_IDENTIFIER__</string>\n\t</array>');
    //log('Successfully added Keychain Sharing capability to the app!', 'success');
    ...
    //Entitlements - Enabling Siri support
    plistContents = plistContents.replace(/<dict>/g, '<dict>\n\t<key>com.apple.developer.siri</key>\n\t<true/>');
    log('Successfully added Siri capability to the app!', 'success');

    //Entitlements - Enabling Keychain Sharing support 
    //plistContents = plistContents.replace(/<dict>/g, '<dict>\n\t<key>keychain-access-groups</key>\n\t<array>\n\t\t<string>$(AppIdentifierPrefix)__KEYCHAIN_GROUP__</string>\n\t</array>');
    //log('Successfully added Siri capability to the app!', 'success');
